...
def main(path, start_index=0, end_index=None, openai_api_base=None, save_path=None, save_path_error=None):
    ...
    # 判断过滤掉 model_gene为空 的重新生成
    cur_datas = [elem for elem in datas if elem['model_gene'] == '']
    ...
    for idx in range(0, len(cur_datas), 200000):
        start = idx
        end = min(start+200000, len(cur_datas))
...
# 处理结果，提取其中，model_gene 不为空，且res 为True 的用于训练
def deal_res(path, save_path):
    with open(save_path, 'a', encoding='utf8') as f_save, open('ic_errors.jsonl', 'a', encoding='utf8') as f2,  open('ic_errors2.jsonl', 'a', encoding='utf8') as f3:
        for elem in yield_data(path):
            if elem['model_gene']:
                try:
                    if json.loads(elem['model_gene'])['res']:
                        # 保存到指定路径
                        f_save.write(json.dumps({'messages': elem['messages']}, ensure_ascii=False)+'\n')
                    else:
                        f2.write(json.dumps(elem, ensure_ascii=False)+'\n')
                except:
                    f3.write(json.dumps(elem, ensure_ascii=False)+'\n')

                        

def deal_res_deepcoder(path, save_path):
    with open(save_path, 'a', encoding='utf8') as f_save:
        for elem in yield_data(path):
            messages = {
                "messages": [
                    {"role": "user", "content": elem['messages'][0]['content']},
                    {"role": "assistant", "content": elem['model_gene']}
                ]
            }
...
if __name__ == '__main__':
    # 过滤 IC、Ner中 语言混乱的样本
    ori_path = '/mnt/code/zhaoxudong03/data_pipelines/training_data/457w/datas_domain_ic_dep_lst_70w_format.jsonl'
    ori_path = '/mnt/code/zhaoxudong03/data_pipelines/training_data/457w/datas_domain_ic_dep_lst_70w_format_true.jsonl'
    save_path = '/mnt/code/zhaoxudong03/data_pipelines/training_data/457w/datas_domain_ic_dep_lst_70w_format_true2.jsonl'
    save_path_error = '/mnt/code/zhaoxudong03/data_pipelines/training_data/457w/datas_domain_ic_dep_lst_70w_format_false.jsonl'
    # 1、生成response
    start_index = 0
    end_index = None
    ...
    # path = '/mnt/code/zhaoxudong03/RL/verl/datas/deepcoder/deepcoder_train_modified_swift_122b_distill_1times_error_for_pred_buchong.jsonl'
    # deal_res_deepcoder(path, save_path='/mnt/code/zhaoxudong03/data_pipelines/training_data/457w/deepcoder_deal_by_122b.jsonl')

    # 5、处理ic 的数据 
    deal_res('/mnt/code/zhaoxudong03/data_pipelines/training_data/457w/datas_domain_ic_dep_lst_70w_format_true.jsonl', '/mnt/code/zhaoxudong03/data_pipelines/training_data/457w/domain_ic_70w_filter_122B.jsonl')
    deal_res('/mnt/code/zhaoxudong03/data_pipelines/training_data/457w/datas_domain_ic_dep_lst_70w_format_true2.jsonl', '/mnt/code/zhaoxudong03/data_pipelines/training_data/457w/domain_ic_70w_filter_122B.jsonl')
