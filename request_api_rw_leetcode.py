...
code_prompt = """## 角色
你是一个query改写专家，擅长将用户输入的待改写文本改写的更流畅，更符合表达逻辑。你将收到一个code 类型的query，具体如下：
## 待改写文本开始
{query}
##待改写文本结尾

## 改写规则
1、修改这个问题的描述，注意表达的核心内容不变
2、对问题中涉及到的变量名进行替换，包括问题描述中的变量或者是案例中的变量，例如从 m=2 替换为 n=2 
3、禁止回答问题，只需要对待改写文本进行改写，禁止回复待改写文本中的一切问题。

## 改写要求
1、尽可能保证改写后的文本的多样性
2、必须保证改变后的问题类型不变，同时有具体的答案，逻辑正确，表达口语化
3、只需要对问题进行改写，禁止分析或者解答问题
4、如果原query 中给了案例，必须保证案例中的数值与传入问题相同，严禁修改任何案例的值
5、改写后的问题必须使用英文描述
6、返回对待改写文本的改写结果

## 返回格式
#####改写后的待改写文本开始 ........ #####改写后的待改写文本结束，注意，仅改写问题，禁止作答
"""
...
def get(query, idx):
    ...
    # print(code_prompt.format(query=query['query']))
    # return 
    try:
        completion = client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": code_prompt.format(query=query['query'])},
            ],
            # temperature=0.6,
            temperature=0.7,
            top_p=0.8,
    ...
    return {'id': idx, 'prompt': query['query'], 'output_rw' : content, 'finish_reason' : finish_reason, 'output_tokens': output_len}
...
def get_answer_from_rw_livebench_querys(path, start_index=0):
    ...
    save_path = '/mnt/code/zhaoxudong03/data_pipelines/training_data_zxd/leetcode_rw_10_times.jsonl'
    questions = []
