...
class GlobalEmbeddingDeduplicator(BaseDeduplicator):
    ...
    def deduplicate(self):
        """主入口：执行全局去重并保存结果（支持LSH优化路由）"""
        # 检查是否启用LSH优化
        use_lsh = getattr(self.config, 'use_lsh', False)
        
        if use_lsh:
            # 使用LSH优化版本
            # 注意：如果已经是LSH实例，直接使用；否则需要转换
            if self.__class__.__name__ == 'GlobalEmbeddingLSHDeduplicator':
                # 已经是LSH实例，直接调用
                unique_data, stats = self.global_deduplicate()
            else:
                # 需要使用LSH版本，但不重新创建实例以避免重复初始化
                print("🚀 检测到use_lsh=True，启用LSH优化模式")
                print("💡 建议：直接使用GlobalEmbeddingLSHDeduplicator类以获得更好的性能")
                try:
                    # 临时导入LSH版本的global_deduplicate方法
                    from global_embedding_lsh_dedup import GlobalEmbeddingLSHDeduplicator
                    # 使用组合而非继承，避免重复初始化
                    # 复制必要的属性到一个新实例
                    lsh_instance = object.__new__(GlobalEmbeddingLSHDeduplicator)
                    # 复制当前实例的所有属性
                    lsh_instance.__dict__.update(self.__dict__)
                    # 设置LSH特定属性
                    lsh_instance.use_lsh = True
                    lsh_instance.lsh_num_tables = getattr(self.config, 'lsh_num_tables', 10)
                    lsh_instance.lsh_hash_size = getattr(self.config, 'lsh_hash_size', 10)
                    unique_data, stats = lsh_instance.global_deduplicate()
                except ImportError as e:
                    print(f"⚠️  无法导入LSH优化版本: {e}")
                    print("🔄 回退到精确模式")
                    unique_data, stats = self.global_deduplicate()
        else:
            # 使用精确FAISS检索模式
