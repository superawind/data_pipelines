...
class GlobalStrictHashDeduplicator(BaseDeduplicator):
    ...
    def deduplicate(self):
        ...
        with open(self.output_file, 'w', encoding='utf-8') as f:
            for line_idx, line in tqdm(unique_data, desc="💾 写入结果"):
                # unique_data是(line_idx, line)的列表，line已经是JSON字符串
                # 需要先解析成dict，然后使用field_order排序后重新写入
                try:
                    data = json_loads(line)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="全局严格哈希去重器")
