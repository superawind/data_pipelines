...
def get(query, idx):
    ...
    if idx % 2 == 0:
        openai_api_base = 'http://172.8.127.195:8027/v1'
    else:
        openai_api_base = 'http://172.8.127.195:8027/v1'
    ...
    return {'id': idx, 'source': query['source'], 'prompt': query['prompt'], '235b_Res' : query['output'],'8b_Res': content, 'score': query['score'], 'finish_reason' : finish_reason, 'output_tokens': output_len, 'score': query['score']}
...
def get_answer_from_rw_livebench_querys(path, start_index=0):
    ...
    save_path = '/code/zhaoxudong03/data_pipelines/training_data_zxd/res_8B/Code.jsonl'
    questions = []
    ...
    for line in tqdm(yied_datas(path)):
        sample = json.loads(line)
        # query = sample['prompt']
        questions.append(sample)
...
if __name__ == '__main__':
    ...
    get_answer_from_rw_livebench_querys('/code/zhaoxudong03/data_pipelines/training_data_zxd/step2/res_clarity_code_not_over_8_to_10_and_cg_over_8_to_10/Code.jsonl')
